...
parser = argparse.ArgumentParser(description="Evaluate model on job title prediction")
...
def extract_target_job_title(text):
    match = re.search(r'target job title:\s*"([^"]+)"', text)
    return match.group(1) if match else None


class JobPredictionDataset(Dataset):
    def __init__(self, data_path, tokenizer):
        with open(data_path, 'r') as f:
            self.data = json.load(f)
        self.tokenizer = tokenizer
        self.system_prompt = (
            "You are a professional job recommendation expert. Given a user’s Career History and Open Source Project Experience "
            "(represented by corresponding APIs in the open-source project), you must determine whether the user is likely to work on the target job title."
        )

    def get_prompt(self, career_history, project, target_job_title):
        return (
            f'Career History: "{career_history}"\n'
            f'Open Source Project Experience: "{project}"\n'
            f'Analyze user\'s career history and open source project experience to determine whether user is likely to work on the target job title:"{target_job_title}"\n\n'
    ...
    def __getitem__(self, idx):
        ...
        target_job_title = extract_target_job_title(input_text)

        prompt = self.get_prompt(career_history, project_experience, target_job_title)
        messages = [
...
dataset = JobPredictionDataset(args.data_path, tokenizer)
